...
const createUser = async (name, email, password, role) => {
  ...
  if (error) {
    return {
      status: BAD_REQUEST_STATUS, err: { message: 'Invalid entries. Try again.' },
    }; 
}
  ...
  if (repeatedEmail) {
    return {
      status: CONFLICT_STATUS, err: { message: 'Email already registered' },
    }; 
}
...
const createToken = async (data) => {
  ...
  if (error) return { status: UNAUTHORIZED_STATUS, err: { message: 'All fields must be filled' } };
