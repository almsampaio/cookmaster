...
const imagemDaReceita = async (req, res) => {
  const { id } = req.params;
  const { _id: userId, role } = req.user;
  const { path: image } = req.file;
//   console.log('req ', req.file);
  const imagem = await receitasModel.imagemDaReceita(id, userId, role, image);

  if (imagem === null) return res.status(404).json({ message: 'recipe not found' });
  if (imagem === false) return res.status(401).json({ message: 'missing auth token' });
